  ...
  try:
    while not_reached_nsteps:
      ...
      for example in dl:  # ((position, particle_type, material_property, n_particles_per_example), labels) are in dl
        ...
        loss = (pred_acc - target_acc) ** 2 # Loss is n*t accelerations
        loss = loss.sum(dim=-1) # Find average acceleration loss of each particle across t
        num_non_kinematic = non_kinematic_mask.sum() # Sum up the number of particles moving
        loss = torch.where(non_kinematic_mask.bool(),
                         loss, torch.zeros_like(loss)) # Find the loss of the particles that are classified as moving ("non_kinematic_mask" -bad)
        loss = loss.sum() / num_non_kinematic # Loss is the average acceleration loss of the moving particles across time.
